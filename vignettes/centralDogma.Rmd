...
The first step of the central dogma is to translate a gene (DNA strand) into RNA
```{r }
...
In reality, some post translational changes occur to the messenger RNA, but we will ignore that fact here.
To properly transcribe the RNA, it needs to be converted into a list of codons.
```{r }
...
The list of codons can then be transcribed using the `codon_table` provided with the package.
Run `?codon_table` if you want more information.
```{r }
peptide <- transcribe(codons = codons)
```

A lot of post-transcriptional changes are performed to the peptide, but this package will not apply those.
